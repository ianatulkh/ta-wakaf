...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Data Saksi</h3>
    </div>
    ...
    <div class="card-body table-responsive mr-2">
        ...
        <div class="alert alert-info alert-dismissible">
            ...
            Data saksi yang telah memiliki data akta ikrar (sudah menjadi saksi di beberapa ikrar wakaf) tidak dapat dihapus, karena untuk kepentingan arsip KUA
        </div>

        <a href="{{ route('admin.data-saksi.create') }}" class="btn btn-primary mb-2">Tambah Saksi</a>
...
$(function () {
    $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'), } });
     ...
     let table = $('.dataTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.data-saksi.index') }}",
        columns: [
            ...
            {data: 'action', name: 'action', orderable: false, searchable: false,
                render: function( data, _type, _full ) {
                    let btn = null;
                    let URL_SHOW = '{{ route("admin.data-saksi.show", ":id") }}';
                    let URL_EDIT = '{{ route("admin.data-saksi.edit", ":id") }}';
                    let URL_DESTROY = '{{ route("admin.data-saksi.destroy", ":id") }}';
